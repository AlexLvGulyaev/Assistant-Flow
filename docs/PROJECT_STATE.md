...
- **Не решено:** semantic/glossary-aware chunking (см. SPEC § «Вне скоупа»).
...
- **Коммерческие риски:** single-tenant (нет multi-tenant изоляции), память VPS — swap почти полон на простое (состояние машины от других стеков), нет CI/CD и автоматических бэкапов.
...
| 2026-09-15 | Разработка | rag_smoke_test подключён к tuning-резолверу (RetrievalTuningResolver — как прод-контур): smoke-тест теперь проверяет effective-настройки (DB override → env → код), а не сырой env-конфиг |
| 2026-09-15 | Сопровождение | Тезис «heavy RAG деградирует на малых VPS» снят как не подтверждённый текущей конфигурацией: живой прогон 15.09 (prod async-reindex 419 чанков за 21 с + 3 конкурентных RAG-потока, 27 запросов, mean 9.4 с без деградации латентности и памяти; OOM нет). Первоисточник тезиса — инцидент эпохи VPS ~2 ГБ/no-swap; на AIP при 18-кратном корпусе (7656 чанков) зафиксированы проблемы корректности синка, не ресурсы. Остаточный риск — swap хоста почти полон на простое |
